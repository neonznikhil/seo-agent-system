...
async def run_content_strategy_async(website_id: str, query: str) -> str:
    SERPAnalyzerTool = _get_serp_tool_cls()
    seo_aeo_geo_tool = _get_seo_tool()
    ...
    website = (
        get_supabase().table("websites").select("domain,cms_url").eq("id", website_id).single().execute().data
        or {}
    )
...
async def run_full_site_optimization_async(website_id: str) -> dict:
    ...
    try:
        ...
        if website.get("gsc_property"):
            gsc_keywords = (
                get_supabase().table("gsc_keywords").eq("website_id", website_id).limit(20).execute().data or []
            )
            urls.extend(
                [f"https://{website['domain']}/blog/{kw['query'].replace(' ', '-')}" for kw in gsc_keywords[:5]]
            )
        ...
        if urls:
            ...
            for item in audit_data:
                ...
                suggestions = (
                    analysis.get("seo_suggestions", [])
                    + analysis.get("aeo_suggestions", [])
                    + analysis.get("geo_suggestions", [])
                )
            ...
            results["content_plan"] = [
                {
                    ...
                    "type": "pillar",
                    "target_keyword": f"how to {website.get('domain', '').split('.')[0]}",
                },
                {
                    "topic": "Advanced strategies",
                    "type": "blog",
                    "target_keyword": f"{website.get('domain', '').split('.')[0]} advanced",
                },
                {
                    "topic": "Case studies",
                    "type": "case-study",
                    "target_keyword": f"{website.get('domain', '').split('.')[0]} case study",
                },
            ]
